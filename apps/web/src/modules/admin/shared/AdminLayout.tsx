...
export default function AdminLayout() {
  ...
  // Identitas akun yang SEBENARNYA sedang login. Sebelumnya kotak ini
  // mencetak "Administrator / admin@wedding.id" secara hardcode, sehingga
  // petugas gate pun melihat dirinya disebut Administrator - satu-satunya
  // tempat di dasbor yang menyebut siapa yang login justru menyebut orang
  // yang salah, tepat di atas tombol Keluar.
  //
  // username null = sesi lama yang tersimpan sebelum field ini ada; peran
  // tetap ditampilkan sehingga kotaknya tidak pernah kosong.
  const accountName = username ?? 'Akun admin'
  const accountRoleLabel = isScannerRole(role) ? 'Petugas gate' : 'Admin penuh'
  const accountInitial = (username ?? 'A').trim().charAt(0).toUpperCase() || 'A'
    ...
    <div className="min-h-screen flex bg-[#f8fafc] text-slate-800 font-sans antialiased">
        ...
        <div className="p-3.5 border-t border-slate-800/80 bg-slate-950/40">
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-slate-900/60 border border-slate-800/60 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-200 truncate">{accountName}</p>
              <p className="text-[11px] text-slate-400 truncate">{accountRoleLabel}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200/80 px-4 sm:px-8 flex items-center justify-between shrink-0 shadow-2xs">
          ...
          <div className="flex items-center gap-3">
            {/* Mengantar ke menu Tamu, BUKAN ke "/" seperti dulu.
                Undangan kini digerbangi token tamu (GuestGate): membuka "/"
                polos hanya membuahkan layar "Anda tidak terdaftar", bahkan bagi
                admin - tombol yang menolak pemiliknya sendiri. Pratinjau yang
                benar adalah membuka tautan SEORANG tamu, dan tombol "Salin link"
                per baris di menu Tamu sudah menyediakannya. Sengaja TIDAK
                target="_blank": ini navigasi di dalam dasbor, bukan membuka
                undangan.

                Disembunyikan dari petugas gate karena menu Tamu bukan haknya -
                ProtectedRoute akan memantulkannya balik ke Scan, dan tombol yang
                memantul persis masalah yang sudah diperingatkan di
                route-paths.ts.

                `Link`, BUKAN `NavLink`: NavLink menandai dirinya
                aria-current="page" saat rutenya cocok, sehingga di /guests ada
                DUA elemen yang sama-sama mengaku halaman aktif - pembaca layar
                jadi menerima dua jawaban untuk satu pertanyaan. Tombol ini
                pintasan tindakan, bukan navigasi utama; yang menandai halaman
                aktif adalah sidebar. */}
            {!isScannerRole(role) && (
            <Link
              to={ROUTE_PATHS.guests}
